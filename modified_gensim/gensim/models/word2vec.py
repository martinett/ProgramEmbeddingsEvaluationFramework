...
from modified_gensim.gensim.utils import keep_vocab_item, call_on_class_only, deprecated
from modified_gensim.gensim.models.keyedvectors import KeyedVectors, pseudorandom_weak_vector
from modified_gensim.gensim import utils, matutils

# This import is required by pickle to load models stored by Gensim < 4.0, such as Gensim 3.8.3.
from modified_gensim.gensim.models.keyedvectors import Vocab  # noqa
...
try:
    from modified_gensim.gensim.models.word2vec_inner import (  # noqa: F401
        train_batch_sg,
        train_batch_cbow,
        train_batch_cbow_concat,
        score_sentence_sg,
        score_sentence_cbow,
        MAX_WORDS_IN_BATCH,
        FAST_VERSION,
...
try:
    from modified_gensim.gensim.models.word2vec_corpusfile import train_epoch_sg, train_epoch_cbow, CORPUSFILE_VERSION
except ImportError:
...
class Word2Vec(utils.SaveLoad):
    def __init__(
            ...
            sg=0, hs=0, negative=5, ns_exponent=0.75, cbow_mean=1, cbow_concat=0, hashfxn=hash, epochs=5, null_word=0,
            trim_rule=None, sorted_vocab=1, batch_words=MAX_WORDS_IN_BATCH, compute_loss=False, callbacks=(),
            comment=None, max_final_vocab=None, shrink_windows=True,
            force_log_exp=False, compute_epoch_0_var=False, init_inter_with_0s=True, #nb_word_losses=0 #MODIF
            batch_documents=None, normalize_initial_wv=True #MODIF
        ):
        ...
        self.cbow_concat = int(cbow_concat)
        ...
        self.normalize_initial_wv = normalize_initial_wv
        self.rejected_by_max_exp = 0 #MODIF
        self.init_inter_with_0s = init_inter_with_0s
        
        self.force_log_exp = force_log_exp #MODIF
        self.compute_epoch_0_var = compute_epoch_0_var
        if self.compute_epoch_0_var:
            self.wv_vars_0 = []
            self.syn_vars_0 = []
        # self.word_wise_loss = bool(nb_word_losses) #MODIF
        # self.nb_word_losses = nb_word_losses #MODIF
        # self.word_losses = np.zeros((nb_word_losses+1, self.epochs)) #MODIF

        if not hasattr(self, 'wv'):  # set unless subclass already set (eg: FastText)
            self.wv = KeyedVectors(vector_size, init_normalize=self.normalize_initial_wv)
        ...
        if not hasattr(self, 'layer1_size'):  # set unless subclass already set (as for Doc2Vec dm_concat mode)
            if self.cbow_concat:
                self.layer1_size = (2 * window) * vector_size
            else:
    ...
    def init_weights(self):
        ...
        if self.init_inter_with_0s:
            syn = np.zeros((len(self.wv), self.layer1_size), dtype=REAL)
        else:
            syn = (np.random.random((len(self.wv), self.layer1_size)) * 2 - 1).astype(REAL)
        if self.hs:
            self.syn1 = syn
        if self.negative:
            self.syn1neg = syn
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        elif self.cbow_concat:
            tally += train_batch_cbow_concat(self, sentences, alpha, work, neu1, self.compute_loss)
        else:
    ...
    def _worker_loop(self, job_queue, progress_queue, cur_epoch):
        ...
        while True:
            ...
            if cur_epoch == 0 and self.compute_epoch_0_var:
                wv_var = np.var(self.wv.vectors)
                self.wv_vars_0.append(wv_var)
                if self.hs:
                    syn_var = np.var(self.syn1)
                if self.negative:
                    syn_var = np.var(self.syn1neg)
                self.syn_vars_0.append(syn_var)
    ...
    def _job_producer(self, data_iterator, job_queue, cur_epoch=0, total_examples=None, total_words=None):
        ...
        for data_idx, data in enumerate(data_iterator):
            ...
            if batch_size + data_length <= self.batch_words and (self.batch_documents is None or job_documents < self.batch_documents):
    ...
    def _train_epoch_corpusfile(
        ...
        **kwargs : object
        ...
        from modified_gensim.gensim.models.word2vec_corpusfile import CythonVocab
        from modified_gensim.gensim.models.fasttext import FastText
    ...
    def _train_epoch(
        ...
        workers = [
            threading.Thread(
                ...
                args=(job_queue, progress_queue, cur_epoch))
            for _ in range(self.workers)
        ]
    ...
    def reset_from(self, other_model):
        ...
        """
        self.wv = KeyedVectors(self.vector_size, init_normalize=self.normalize_initial_wv)
    ...
    def get_latest_rejected_by_max_exp(self): #MODIF
        """Get current value of the training loss.
        ...
        """
        return self.rejected_by_max_exp
...
if __name__ == "__main__":
    ...
    from modified_gensim.gensim.models.word2vec import Word2Vec  # noqa:F811 avoid referencing __main__ in pickle
