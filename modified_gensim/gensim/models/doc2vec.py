...
from modified_gensim.gensim import utils, matutils  # utility fnc for pickling, common scipy operations etc
from modified_gensim.gensim.utils import deprecated
from modified_gensim.gensim.models import Word2Vec, FAST_VERSION  # noqa: F401
from modified_gensim.gensim.models.keyedvectors import KeyedVectors, pseudorandom_weak_vector

logger = logging.getLogger(__name__)

try:
    from modified_gensim.gensim.models.doc2vec_inner import train_document_dbow, train_document_dm, train_document_dm_concat
except ImportError:
    raise utils.NO_CYTHON

try:
    from modified_gensim.gensim.models.doc2vec_corpusfile import (
...
class Doc2Vec(Word2Vec):
    def __init__(
            ...
            window=5, epochs=10, shrink_windows=True,
            learn_words=True, learn_tags=True, learn_syn1=True, #MODIF
        ...
        self.learn_tags = learn_tags
        self.learn_syn1 = learn_syn1
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for doc in job:
            ...
            doctag_vectors = self.dv.vectors
            if not self.tags_locks is None:
                doctags_lockf = self.tags_locks
            else:
                doctags_lockf = self.dv.vectors_lockf
            words_lockf = self.words_locks
            syn1_lockf = self.syn1_locks
            
            if self.sg:
                tally += train_document_dbow(
                    self, doc.words, doctag_indexes, alpha,
                    self.compute_loss, syn1_lockf, #MODIF
                    work, train_words=self.dbow_words,
                    doctag_vectors=doctag_vectors, doctags_lockf=doctags_lockf,
                    learn_doctags=self.learn_tags, learn_words=self.learn_words, learn_hidden=self.learn_syn1, #MODIF
                    words_lockf=words_lockf, #MODIF
                    window_slide=self.window_slide #MODIF
                )
            elif self.dm_concat:
    ...
    def train(
        ...
        words_locks=None, tags_locks=None, syn1_locks=None, #MODIF
        **kwargs,
    ):
        ...
        """
        if words_locks is None:
            words_locks = np.ones(1, dtype=REAL)
        self.words_locks = words_locks
        
        if tags_locks is None:
            tags_locks = np.ones(1, dtype=REAL)
        self.tags_locks = tags_locks
        
        if syn1_locks is None:
            syn1_locks = np.ones(1, dtype=REAL)
        self.syn1_locks = syn1_locks
    ...
    def infer_vector(self, doc_words, alpha=None, min_alpha=None, epochs=None):
        ...
        for i in range(epochs):
            if self.sg:
                train_document_dbow(
                    self, doc_words, doctag_indexes, alpha,
                    self.compute_loss, None, #MODIF
                    work,
                    learn_words=False, learn_hidden=False, doctag_vectors=doctag_vectors, doctags_lockf=doctags_lockf, #MODIF
