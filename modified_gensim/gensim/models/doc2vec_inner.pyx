...
import cython
import numpy as np
from numpy import zeros, ones, float32 as REAL
cimport numpy as np
...
from libc.stdio cimport printf #MODIF
...
from modified_gensim.gensim.models.word2vec_inner cimport bisect_left, random_int32, sscal, REAL_t, EXP_TABLE, our_dot, our_saxpy
from modified_gensim.gensim.models.word2vec_inner cimport LOG_TABLE #MODIF
...
cdef void fast_document_dbow_hs(
    ...
    REAL_t *contexts_lockf, const np.uint32_t contexts_lockf_len,
    REAL_t *syn1_lockf, const np.uint32_t syn1_lockf_len, #MODIF
    const int _compute_loss, REAL_t *_running_training_loss_param, #MODIF
    np.uint32_t *_rejected_by_max_exp_param, int force_log_exp #MODIF
    ) nogil:
    # printf("start fast_document_dbow_hs\n")

    cdef long long a, b
    cdef long long row1 = context_index * size, row2, sgn
    cdef REAL_t f, g, f_dot, lprob, syn1_corr
    ...
    for b in range(codelen):
        ...
        if learn_hidden:
            syn1_corr = syn1_lockf[row2 % syn1_lockf_len] * g
            our_saxpy(&size, &syn1_corr, &context_vectors[row1], &ONE, &syn1[row2], &ONE)
    if learn_context:
        our_saxpy(&size, &contexts_lockf[context_index % contexts_lockf_len],
                  work, &ONE, &context_vectors[row1], &ONE)
    # printf("end fast_document_dbow_hs\n")
...
cdef unsigned long long fast_document_dbow_neg(
    ...
    const np.uint32_t contexts_lockf_len,
    REAL_t *syn1_lockf, const np.uint32_t syn1_lockf_len, #MODIF
    const int _compute_loss, REAL_t *_running_training_loss_param, #MODIF
    np.uint32_t *_rejected_by_max_exp_param, int force_log_exp #MODIF
    ) nogil:
    # printf("start fast_document_dbow_neg\n")

    cdef long long a
    cdef long long row1 = context_index * size, row2
    cdef unsigned long long modulo = 281474976710655ULL
    cdef REAL_t f, g, label, f_dot, log_e_f_dot, syn1_corr
    ...
    for d in range(negative+1):
        ...
        if learn_hidden:
            syn1_corr = syn1_lockf[row2 % syn1_lockf_len] * g
            our_saxpy(&size, &syn1_corr, &context_vectors[row1], &ONE, &syn1neg[row2], &ONE)
    if learn_context:
        our_saxpy(&size, &contexts_lockf[context_index % contexts_lockf_len],
                  work, &ONE, &context_vectors[row1], &ONE)

    # printf("end fast_document_dbow_neg\n")
    return next_random
...
cdef void fast_document_dm_hs(
    ...
    # printf("start fast_document_dm_hs\n")
    ...
    for b in range(word_code_len):
        ...
        if learn_hidden:
            syn1_corr = syn1_lockf[row2 % syn1_lockf_len] * g
            our_saxpy(&size, &syn1_corr, neu1, &ONE, &syn1[row2], &ONE)
    # printf("end fast_document_dm_hs\n")
...
cdef unsigned long long fast_document_dm_neg(
    ...
    # printf("start fast_document_dm_neg\n")
    ...
    for d in range(negative+1):
        ...
        if learn_hidden:
            syn1_corr = syn1_lockf[row2 % syn1_lockf_len] * g
            our_saxpy(&size, &syn1_corr, neu1, &ONE, &syn1neg[row2], &ONE)

    # printf("end fast_document_dm_neg\n")
    return next_random

cdef void fast_document_dmc_hs(
    const np.uint32_t *word_point, const np.uint8_t *word_code, int word_code_len,
    REAL_t *neu1, REAL_t *syn1, const REAL_t alpha, REAL_t *work,
    const int layer1_size, const int vector_size, int learn_hidden,
    REAL_t *syn1_lockf, const np.uint32_t syn1_lockf_len, #MODIF
    const int _compute_loss, REAL_t *_running_training_loss_param, #MODIF
    np.uint32_t *_rejected_by_max_exp_param, int force_log_exp #MODIF
    ) nogil:
    # printf("start fast_document_dmc_hs\n")
    ...
    for b in range(word_code_len):
        ...
        if learn_hidden:
            syn1_corr = syn1_lockf[row2 % syn1_lockf_len] * g
            our_saxpy(&layer1_size, &syn1_corr, neu1, &ONE, &syn1[row2], &ONE)
    # printf("end fast_document_dmc_hs\n")
...
cdef unsigned long long fast_document_dmc_neg(
    ...
    # printf("start fast_document_dmc_neg\n")
...
cdef init_d2v_config(Doc2VecConfig *c, model, alpha, learn_doctags, learn_words, learn_hidden,
    ...
    c[0].compute_loss = (1 if compute_loss else 0) #MODIF
    c[0].running_training_loss = model.running_training_loss #MODIF
    ...
    c[0].window_slide = (1 if window_slide else 0) #MODIF
    ...
    c[0].syn1_lockf = <REAL_t *>(np.PyArray_DATA(syn1_lockf))
    c[0].syn1_lockf_len = len(syn1_lockf)
    ...
    # printf("end init_d2v_config\n")



def train_document_dbow(model, doc_words, doctag_indexes, alpha,
                        compute_loss, syn1_lockf, #MODIF
                        work=None,
    ...
    """
    # printf("start train_document_dbow\n")
    ...
    init_d2v_config(&c, model, alpha, learn_doctags, learn_words, learn_hidden,
                    compute_loss, syn1_lockf, #MODIF
                    train_words=train_words, work=work,
                    neu1=None, word_vectors=word_vectors, words_lockf=words_lockf,
                    doctag_vectors=doctag_vectors, doctags_lockf=doctags_lockf,
                    window_slide=window_slide #MODIF
                    )
    ...
    if c.train_words:
        ...
        if model.shrink_windows and c.window > 0:
    ...
    with nogil:
        # printf("syn1_lockf: %f\n", c.syn1_lockf[0])
        # printf("words_lockf: %f\n", c.words_lockf[0])
        # printf("doctags_lockf: %f\n\n", c.doctags_lockf[0])
        for i in range(c.document_len):
                ...
                for j in range(j, k):
                    ...
                    if c.hs:
                        ...
                        fast_document_dbow_hs(c.points[i], c.codes[i], c.codelens[i], c.word_vectors, c.syn1, c.layer1_size,
                                              ...
                                              c.words_lockf_len,
                                              c.syn1_lockf, c.syn1_lockf_len, #MODIF
                                              c.compute_loss, &c.running_training_loss, #MODIF
                                              &c.rejected_by_max_exp, c.force_log_exp #MODIF
                                              )
                    if c.negative:
                        # we reuse the DBOW function, as it is equivalent to skip-gram for this purpose
            ...
            if not c.window_slide:
                break

    model.running_training_loss = c.running_training_loss #MODIF
    model.rejected_by_max_exp = c.rejected_by_max_exp #MODIF
    # printf("end train_document_dbow\n")
    return result


def train_document_dm(model, doc_words, doctag_indexes, alpha,
                      compute_loss, syn1_lockf, #MODIF
                      work=None, neu1=None,
    ...
    """
    # printf("start train_document_dm\n")
    cdef Doc2VecConfig c
...
#        vocab_sample_ints = model.wv.expandos['sample_int']  # this variant noticeably slower
    ...
    model.running_training_loss = c.running_training_loss #MODIF
    model.rejected_by_max_exp = c.rejected_by_max_exp #MODIF
    # printf("end train_document_dm\n")
    return result


def train_document_dm_concat(model, doc_words, doctag_indexes, alpha,
                             compute_loss, syn1_lockf, #MODIF
                             work=None, neu1=None,
                             learn_doctags=True, learn_words=True, learn_hidden=True,
                             word_vectors=None, words_lockf=None, doctag_vectors=None, doctags_lockf=None,
    ...
    """
    # printf("start train_document_dm_concat\n")
    cdef Doc2VecConfig c
    ...
    init_d2v_config(&c, model, alpha, learn_doctags, learn_words, learn_hidden,
                    compute_loss, syn1_lockf, #MODIF
                    train_words=False, work=work, neu1=neu1,
    ...
    model.running_training_loss = c.running_training_loss #MODIF
    model.rejected_by_max_exp = c.rejected_by_max_exp #MODIF
    # printf("end train_document_dm_concat\n")
    return result
