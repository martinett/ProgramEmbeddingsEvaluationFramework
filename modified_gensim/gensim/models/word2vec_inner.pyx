...
cdef void w2v_fast_sentence_sg_hs(
    ...
    const np.uint32_t lockf_len, const int _compute_loss, REAL_t *_running_training_loss_param,
    np.uint32_t *_rejected_by_max_exp_param, int force_log_exp, int learn_vectors #MODIF
    ) nogil:
    ...
    for b in range(codelen):
        ...
        if f_dot <= -MAX_EXP or f_dot >= MAX_EXP:
            if force_log_exp == 1:
                ...
                _rejected_by_max_exp_param[0] += 1 #MODIF
                continue
        else:
        ...
        if _compute_loss == 1:
            ...
            if lprob <= -MAX_EXP or lprob >= MAX_EXP:
                if force_log_exp == 1:
                    lprob = <REAL_t>exp(lprob)
                    lprob = <REAL_t>(lprob / (lprob + 1))
                    if lprob > 0:
                        lprob = <REAL_t>log(lprob)
                    else:
                        _rejected_by_max_exp_param[0] += 1 #MODIF
                        continue
                else:
...
cdef unsigned long long w2v_fast_sentence_sg_neg(
    ...
    for d in range(negative+1):
        ...
        if _compute_loss == 1:
            ...
            if f_dot <= -MAX_EXP or f_dot >= MAX_EXP:
                if force_log_exp == 1:
                    log_e_f_dot = <REAL_t>exp(f_dot)
                    log_e_f_dot = <REAL_t>(log_e_f_dot / (log_e_f_dot + 1))
                    if log_e_f_dot > 0:
                        log_e_f_dot = <REAL_t>log(log_e_f_dot)
                    else:
                        _rejected_by_max_exp_param[0] += 1 #MODIF
                        continue
                else:
...
cdef void w2v_fast_sentence_cbow_concat_hs(
    const np.uint32_t *word_point, const np.uint8_t *word_code, int codelens[MAX_SENTENCE_LEN],
    ...
    const np.uint32_t lockf_len, const int _compute_loss, REAL_t *_running_training_loss_param,
    const np.uint32_t null_word_index, int doc_len,
    np.uint32_t *_rejected_by_max_exp_param, int force_log_exp, int learn_vectors #MODIF
    ) nogil:
    """
    Cbow model with hierarchical softmax, but with concatenation as context aggregation
    """
    cdef long long a, b
    cdef long long row2, sgn, idx
    cdef REAL_t f, g, count, inv_count = 1.0, f_dot, lprob
    cdef int m, m_
    ...
    for m in range(j, k):
        if m == i:
            ...
            if m < 0 or m >= doc_len:
                ...
                idx = <long long>indexes[m]
            count += ONEF
            memcpy(&neu1[<long long>m_ * vector_size], &syn0[idx * <long long>vector_size], vector_size * cython.sizeof(REAL_t))
    if count > (<REAL_t>0.5):
        inv_count = ONEF/count

    memset(work, 0, layer1_size * cython.sizeof(REAL_t))
    for b in range(codelens[i]):
        row2 = <long long>word_point[b] * <long long>layer1_size
        ...
        for m in range(j,k):
            if m == i:
                ...
                if m < 0 or m >= doc_len:
                    ...
                    idx = <long long>indexes[m]
                our_saxpy(&vector_size, &words_lockf[idx % lockf_len], &work[<long long>m_ * <long long>vector_size], &ONE, &syn0[idx * <long long>vector_size], &ONE)


cdef unsigned long long w2v_fast_sentence_cbow_concat_neg(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len, int codelens[MAX_SENTENCE_LEN],
    ...
    """
    Cbow model with negative sampling, but with concatenation as context aggregation
    """
    cdef long long a
    cdef long long row2, idx
    cdef unsigned long long modulo = 281474976710655ULL
    cdef REAL_t f, g, count, inv_count = 1.0, label, log_e_f_dot, f_dot
    cdef np.uint32_t target_index, word_index
    cdef int d, m, m_
    ...
    for d in range(negative+1):
        ...
        row2 = <long long>target_index * <long long>layer1_size
        f_dot = our_dot(&layer1_size, neu1, &ONE, &syn1neg[row2], &ONE)
        if f_dot <= -MAX_EXP or f_dot >= MAX_EXP:
            if force_log_exp == 1:
                f = <REAL_t>exp(f_dot)
                f = <REAL_t>(f / (f + 1))
...
cdef init_w2v_config(Word2VecConfig *c, model, alpha, compute_loss, _work, _neu1=None):
    ...
    c[0].rejected_by_max_exp = model.rejected_by_max_exp #MODIF
    c[0].force_log_exp = (1 if model.force_log_exp else 0) #MODIF
    ...
    # c[0].word_wise_loss = model.word_wise_loss #MODIF
    # c[0].nb_word_losses = nb_word_losses #MODIF
    # c[0].words_loss = <REAL_t *>(model.word_losses) #MODIF
    
    if '\0' in model.wv: #MODIF
...
def train_batch_sg(model, sentences, alpha, _work, compute_loss, learn_vectors=True):
    ...
    cdef int learn_vects = int(learn_vectors)
            ...
            for i in range(idx_start, idx_end):
                ...
                for j in range(j, k):
                    if j == i:
                        continue
                    if c.hs:
                        w2v_fast_sentence_sg_hs(c.points[i], c.codes[i], c.codelens[i], c.syn0, c.syn1, c.size, c.indexes[j], c.alpha, c.work, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss, &c.rejected_by_max_exp, c.force_log_exp, learn_vects)
                    if c.negative:
                        c.next_random = w2v_fast_sentence_sg_neg(c.negative, c.cum_table, c.cum_table_len, c.syn0, c.syn1neg, c.size, c.indexes[i], c.indexes[j], c.alpha, c.work, c.next_random, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss, &c.rejected_by_max_exp, c.force_log_exp, learn_vects)

    model.running_training_loss = c.running_training_loss
    model.rejected_by_max_exp = c.rejected_by_max_exp
    return effective_words


def train_batch_cbow(model, sentences, alpha, _work, _neu1, compute_loss, learn_vectors=True):
        ...
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                if k > idx_end:
                    k = idx_end
                if c.hs:
                    w2v_fast_sentence_cbow_hs(c.points[i], c.codes[i], c.codelens, c.neu1, c.syn0, c.syn1, c.size, c.indexes, c.alpha, c.work, i, j, k, c.cbow_mean, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss, &c.rejected_by_max_exp, c.force_log_exp, learn_vects)
                if c.negative:
                    c.next_random = w2v_fast_sentence_cbow_neg(c.negative, c.cum_table, c.cum_table_len, c.codelens, c.neu1, c.syn0, c.syn1neg, c.size, c.indexes, c.alpha, c.work, i, j, k, c.cbow_mean, c.next_random, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss, &c.rejected_by_max_exp, c.force_log_exp, learn_vects)
...
def train_batch_cbow_concat(model, sentences, alpha, _work, _neu1, compute_loss, learn_vectors=True):
    """Update CBOW model (with concatenation) by training on a batch of sentences.
    ...
    cdef int i, j, k
    cdef int effective_words = 0, effective_sentences = 0
    cdef int sent_idx, idx_start, idx_end, doc_len
    cdef np.uint32_t *vocab_sample_ints
    cdef int learn_vects = int(learn_vectors)
    ...
    with nogil:
        for sent_idx in range(effective_sentences):
            idx_start = c.sentence_idx[sent_idx]
            idx_end = c.sentence_idx[sent_idx + 1]
            doc_len = idx_end - idx_start
            for i in range(idx_start, idx_end):
                j = i - c.window
                k = i + c.window + 1
                if c.hs:
                    w2v_fast_sentence_cbow_concat_hs(c.points[i], c.codes[i], c.codelens, c.neu1, c.syn0, c.syn1, c.layer1_size, c.size, c.indexes, c.alpha, c.work, i, j, k, c.cbow_mean, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss,
                    c.null_word_index, doc_len,
                    &c.rejected_by_max_exp, c.force_log_exp, learn_vects)
                if c.negative:
                    c.next_random = w2v_fast_sentence_cbow_concat_neg(c.negative, c.cum_table, c.cum_table_len, c.codelens, c.neu1, c.syn0, c.syn1neg, c.layer1_size, c.size, c.indexes, c.alpha, c.work, i, j, k, c.cbow_mean, c.next_random, c.words_lockf, c.words_lockf_len, c.compute_loss, &c.running_training_loss,
                    c.null_word_index, doc_len,
                    &c.rejected_by_max_exp, c.force_log_exp, learn_vects)
